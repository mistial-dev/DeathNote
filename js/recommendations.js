...
window.DeathNote.recommendations.recommendations = [// Recommendation 1: Too Few Tasks (Risk of Boredom)
    {
        id: "tooFewTasks", condition: (settings) => {
    ...
    {
        id: "tooManyTasks", condition: (settings) => {
            const taskCounts = window.DeathNote.settings.calculateIdealTaskCount(settings);
            return settings.numberOfTasks.value > taskCounts.hard + 1 && (settings.numberOfTasks.value / (taskCounts.hard + 1) > 1.25); // 75% chance not to complete
        }, message: (settings) => {
            const taskCounts = window.DeathNote.settings.calculateIdealTaskCount(settings);
    ...
    {
        id: "lowSpeedHighTasks", condition: (settings) => {
            const taskCounts = window.DeathNote.settings.calculateIdealTaskCount(settings);
            return settings.movementSpeed.value < 0.8 && settings.numberOfTasks.value > taskCounts.ideal + 1;
        }, message: (settings) => {
            const taskCounts = window.DeathNote.settings.calculateIdealTaskCount(settings);
    ...
    {
        id: "nonDefaultRegion", condition: (settings) => {
    ...
    {
        id: "restrictedPlatformTypes", condition: (settings) => {
            return (settings.pcAllowed && !settings.pcAllowed.value) || (settings.ps4Allowed && !settings.ps4Allowed.value);
        }, message: (settings) => {
    ...
    {
        id: "melloDisabled", condition: (settings) => {
    ...
    {
        id: "noKiraFollower", condition: (settings) => {
    ...
    {
        id: "shortRoundsHighInputs", condition: (settings) => {
            return settings.dayNightSeconds && settings.numberOfInputs && settings.dayNightSeconds.value <= 45 && settings.numberOfInputs.value >= 4;
        }, message: (settings) => {
    ...
    {
        id: "longRoundsLowTasksHighSpeed", condition: (settings) => {
            const taskCounts = window.DeathNote.settings.calculateIdealTaskCount(settings);
            return settings.dayNightSeconds && settings.numberOfTasks && settings.movementSpeed && settings.dayNightSeconds.value >= 120 && settings.numberOfTasks.value < taskCounts.ideal && settings.movementSpeed.value >= 1.2;
        }, message: (settings) => {
            const taskCounts = window.DeathNote.settings.calculateIdealTaskCount(settings);
    ...
    {
        id: "highPlayerCountLowTasks", condition: (settings) => {
            const taskCounts = window.DeathNote.settings.calculateIdealTaskCount(settings);
            return settings.maximumPlayers && settings.numberOfTasks && settings.maximumPlayers.value >= 8 && settings.numberOfTasks.value < taskCounts.ideal;
        }, message: (settings) => {
            const taskCounts = window.DeathNote.settings.calculateIdealTaskCount(settings);
    ...
    {
        id: "noVoiceChatComplexSettings", condition: (settings) => {
            const taskCounts = window.DeathNote.settings.calculateIdealTaskCount(settings);
            return settings.voiceChat && settings.numberOfTasks && settings.numberOfInputs && settings.melloRole && settings.kiraFollowerRole && !settings.voiceChat.value && (settings.numberOfTasks.value > taskCounts.hard || settings.numberOfInputs.value >= 4 || settings.melloRole.value === "random" || settings.kiraFollowerRole.value === "random");
        }, message: (settings) => {
            const taskCounts = window.DeathNote.settings.calculateIdealTaskCount(settings);
    ...
    {
        id: "canvasTasksDisabled", condition: (settings) => {
    ...
    {
        id: "unbalancedProgressMultipliers", condition: (settings) => {
            return settings.kiraProgressMultiplier && settings.teamLProgressMultiplier && Math.abs(settings.kiraProgressMultiplier.value - settings.teamLProgressMultiplier.value) > 0.5;
        }, message: (settings) => {
    ...
    {
        id: "lowNewWorldProgress", condition: (settings) => {
    ...
    {
        id: "highProgressExcessiveJudgments", condition: (settings) => {
            return settings.kiraProgressMultiplier && settings.maximumCriminalJudgments && settings.kiraProgressMultiplier.value >= 1.4 && settings.maximumCriminalJudgments.value > 5;
        }, message: (settings) => {
    ...
    {
        id: "defaultInputsRecommendation", condition: (settings) => {
...
window.DeathNote.recommendations.getActiveRecommendations = function (settings) {
    ...
    return window.DeathNote.recommendations.recommendations.filter(recommendation => {
        ...
        id: recommendation.id, message: recommendation.message(settings)
    }));
};
